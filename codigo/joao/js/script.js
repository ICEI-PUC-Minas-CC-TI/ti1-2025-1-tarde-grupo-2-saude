...
async function loadDoctorsForSpecialty(specialtyId) 
{
    ...
    try 
    {
        const response = await fetch('db/medico.json');
        if (!response.ok) 
        {
...
function setupNameValidation() 
{
    ...
    {
        if (nameInput.value.trim().length < 3 && nameInput.value.trim().length > 0)
        {
...
document.querySelector('form').addEventListener('submit', async function(event) 
{
    ...
    let response = await fetch('db/medico.json');
